class SponsoredpostsController < ApplicationController
  ...
  def update
    @sponsored_post = SponsoredPost.find(params[:id])
    @sponsored_post.title = params[:sponsored_post][:id]
    @sponsored_post.body = params[:sponsored_post][:id]
    @sponsored_post.price = params[:sponsored_post][:id]

    if @sponsored_post.save
      flash[:notice] = "Sponsored post was updated."
      redirect_to [@sponsored_post.topic, @spondored_post]
    else
      flash[:error] = "There was an error saving the sponsored post. Please try again."
      render :edit
    end
